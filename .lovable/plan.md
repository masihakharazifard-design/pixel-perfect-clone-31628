...
- Parent `handleImport` wordt `async`: eerst wordt één volledige nieuwe `next`-array berekend op basis van de huidige projectenlijst (upsert-lus hierboven), daarna `await syncTable("projects", next)`. Pas nadat die sync geslaagd is volgt `const refreshed = await loadAll<...>()` en `setProjects(refreshed.projects)`. `loadAll` wordt nooit vóór de sync aangeroepen, en de bestaande projects-sync-`useEffect` wordt voor de Excel-import niet als opslagpad gebruikt.
