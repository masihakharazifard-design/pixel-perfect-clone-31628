...
5. **Demo-opslag geschikt maken voor grote datasets**
   ...
   - Dunne helperlaag met `openDemoDb()`, `loadDemoProjects()`, `replaceDemoProjects(projects)` en `resetDemoProjects(seed)`. De verbinding wordt één keer geopend en hergebruikt, nooit per project of per rij; `loadDemoProjects()` laadt met één `getAll()`.
   - Excel-import in demo blijft atomair: worker → voorbereide projectregels → volledige nieuwe projectenlijst tijdelijk in memory → één `readwrite`-transactie die binnen dezelfde transactie de store leegt en alle records volgens de importspecificatie schrijft → pas bij `complete` de in-memory projecten vervangen, één `setProjects(next)` en de succesmelding. Bij `abort`/`error` blijft de vorige IndexedDB-toestand behouden, verandert React-state niet, blijft de gebruiker ingelogd en verschijnt alleen een duidelijke foutmelding.
   - `Demo resetten` vervangt de projects-store in één transactie door de actuele seedrecords en werkt pas daarna de React-state bij.
