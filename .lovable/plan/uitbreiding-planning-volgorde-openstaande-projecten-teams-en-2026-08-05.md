...
- `src/components/planning-app.tsx`:
  ...
  - `checkConflicts` geeft een lijst met twee soorten resultaten terug: `blocking` (alleen Bezet/Vakantie/Ziek/Vrij) en `warning` van het type `planning_overlap` (medewerker al aan een ander project gekoppeld in hetzelfde tijdvak). Elk resultaat bevat status, medewerker, datum en tijden. Blokkerend = opslaan geweigerd met melding; `planning_overlap` = bevestigingsvenster "Deze medewerker is op dit tijdstip al aan een ander project gekoppeld." met de knoppen "Planning toch opslaan" en "Annuleren".
