...
5. **Demo-opslag geschikt maken voor grote datasets**
   ...
   - De `projects`-store bevat één record per project met `id` als `keyPath`, plus indexen op `projectnr` en `werknummer`. Nooit één record met de volledige lijst.
   - Normale mutaties raken alleen de betrokken records: toevoegen of wijzigen is één `put`, verwijderen één `delete`, en meerdere gewijzigde werken gaan als losse `put`s binnen één transactie. De volledige lijst wordt daarbij niet herschreven.
   - Bestaat er nog een oude `projects`-array in `maasmond-demo-data`, dan worden die projecten één keer als individuele records binnen één transactie weggeschreven; pas na `transaction.complete` verdwijnt de oude array uit localStorage. Bij een migratiefout blijft de oude data volledig onaangeroerd.
   - Dunne helperlaag met `openDemoDb()`, `loadDemoProjects()`, `putDemoProjects(changed)` en `resetDemoProjects(seed)`. De verbinding wordt één keer geopend en hergebruikt, nooit per project of per rij; `loadDemoProjects()` laadt met één `getAll()`.
   - Excel-import in demo leegt de store nooit. Volgorde: worker levert de voorbereide regels → `handleImport` bouwt met de bestaande `Map<Projectnr., index>` zowel `nextProjects` als `changedProjects` (alleen nieuwe of daadwerkelijk gewijzigde werken) → één `readwrite`-transactie op de `projects`-store → alleen voor `changedProjects` een `put` → wachten op `transaction.complete` → pas dan de in-memory projecten vervangen, één `setProjects(nextProjects)` en de succesmelding. Bij 6.966 bestaande, 200 gewijzigde en 50 nieuwe werken worden dus 250 records geschreven, niet 7.216.
   - Bij `abort`/`error` rolt de transactie alle betrokken wijzigingen terug, blijft React-state ongewijzigd, blijft de gebruiker ingelogd en verschijnt alleen de foutmelding.
   - Alle `put`-requests worden binnen dezelfde transactie gequeued; tijdens een actieve transactie komen er geen `requestAnimationFrame`- of timeout-yields tussen, zodat de transactie niet voortijdig inactive raakt.
   - `clear()` gevolgd door opnieuw schrijven wordt uitsluitend gebruikt voor `Demo resetten`: de store wordt in één transactie vervangen door de actuele seedrecords en pas daarna volgt de React-state.
   - Productiemodus houdt hetzelfde principe aan: alleen nieuwe/gewijzigde werken bulk-upserten, nooit de volledige tabel opnieuw schrijven.
