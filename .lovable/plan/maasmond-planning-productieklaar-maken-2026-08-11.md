...
- RPC's: `patch_app_settings(changes jsonb, paths jsonb default '[]')`. Top-level keys in `changes` worden samengevoegd met `data = COALESCE(data,'{}'::jsonb) || changes`; voor geneste maps (`projectColors`, `deptColors` en vergelijkbare) stuurt de client padgerichte wijzigingen (`[{path: ['projectColors','<id>'], value: ...}]`) die server-side per pad met `jsonb_set(..., create_missing => true)` worden toegepast, zodat gelijktijdige wijzigingen aan verschillende sleutels binnen dezelfde map elkaar niet overschrijven. Arrays (`employeePlanningOrder`, `fixedFreeSeries`) mogen als hele key via `changes` — met een optionele `expected_updated_at` voor de bestaande concurrencycontrole, die bij afwijking de actuele rij teruggeeft in plaats van te overschrijven. Verder transactionele functies zoals `move_planning_transaction(...)`, `delete_planning_transaction(...)`, `set_project_status(...)`, `archive_record(...)`. Alle `SECURITY DEFINER` met `SET search_path = public`, een `auth.uid()`-controle, een rolcontrole en `REVOKE EXECUTE FROM PUBLIC, anon` plus gerichte `GRANT EXECUTE TO authenticated`. Geen enkele functie accepteert een actor of rol als parameter.
